...
class utilisateursManager
{
    ...
    /**
     *
     * @param articles $_utilisateurs
     * @return self
     */
    public function set_article(articles $_utilisateurs): self
    {
    ...
    {
        // Prépare une requête dde tye SELECT avec une classe WHERE selon l'email.
        $sql = 'SELECT * FROM utilisateurs WHERE sid = :sid';
        $req = $this->bdd->prepare($sql);

        // Exécution de la requête avec attribution des valeurs aux marqueurs nominal
        $req->bindValue(':sid', $sid, PDO::PARAM_STR);
        $req->execute();

        //On stocke les donnée obtenues dans un tableau
        $donnee = $req->fetch(PDO::FETCH_ASSOC);

        $donnee = !$donnee ? [] : $donnee;

        $utilisateurs = new utilisateurs();
        $utilisateurs->hydrate($donnee);
        //print_r2($article);
        return $utilisateurs;
    }
    /**
